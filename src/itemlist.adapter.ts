...
export function render_soil_nutrients_html(fertilizer_bonus: FertilizerBonusData) {
    let ret = ''
...
export abstract class ItemListAdapter {
    ...
    static renderSoilNutrientsHtml(fertilizer_bonus: FertilizerBonusData) {
    ...
    static renderShortExpiable(data: any, type: any, row: ItemData): string | boolean {
        if (type === 'display') {
            if (row.expiable) {
    ...
    static renderExpiable(data: any, type: any, row: ItemData): string | boolean {
        if (type === 'display') {
            if (row.expiable) {
